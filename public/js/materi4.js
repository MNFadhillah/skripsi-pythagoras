...
// Logika utama saat klik tombol periksa
function processValidation(section, isComplete, isAllCorrect, btnElement, answerCallback, cpName, successMsg) {
    if (!isComplete) {
    ...
    if (isAllCorrect) {
        Swal.fire({
            ...
            if (typeof updateProgress === 'function') updateProgress('materi_4_penerapan_pythagoras', cpName);
        });
...
function cekApersepsiLengkap() {
    ...
    if (!checkCompleteness(inputs)) return processValidation('ap', false);
...
function showAnswerApersepsi() {
    ...
    if (typeof updateProgress === 'function') updateProgress('materi_4_penerapan_pythagoras', 'm4_cp1_apersepsi');
}
...
function cekContoh1Penerapan() {
    ...
    if (!checkCompleteness(inputs)) return processValidation('c1', false);
    ...
    processValidation('c1', true, isAllCorrect, btn, 'showAnswerC1', 'm4_cp2_contoh_1', 'Langkah penyelesaian Contoh 1 sudah benar.', 10);
}
...
function showAnswerC1() {
    ...
    if (typeof updateProgress === 'function') updateProgress('materi_4_penerapan_pythagoras', 'm4_cp2_contoh_1');
}
...
function cekContoh2Penerapan() {
    ...
    if (!checkCompleteness(inputs)) return processValidation('c2', false);
    ...
    const isAllCorrect = iDik && iDit && iSub && akr && res;
    processValidation('c2', true, isAllCorrect, btn, 'showAnswerC2', 'm4_cp3_contoh_2', 'Perhitungan jarak garis lurus ojek online sangat presisi.', 10);
}
...
function showAnswerC2() {
    ...
    if (typeof updateProgress === 'function') updateProgress('materi_4_penerapan_pythagoras', 'm4_cp3_contoh_2');
}
...
function cekContoh3Penerapan() {
    ...
    if (!checkCompleteness(inputs)) return processValidation('c3', false);
    ...
    processValidation('c3', true, isAllCorrect, btn, 'showAnswerC3', 'm4_cp4_contoh_3', 'Luar Biasa! Contoh 3 dengan 2 segitiga dipahami dengan baik.', 10);
}
...
function showAnswerC3() {
    ...
    if (typeof updateProgress === 'function') updateProgress('materi_4_penerapan_pythagoras', 'm4_cp4_contoh_3');
}
...
function cekSoal1() {
    ...
    if (!checkCompleteness(inputs)) return processValidation('s1', false);
    ...
    const isAllCorrect = iDik && iDit && iSub && akr && res;
    processValidation('s1', true, isAllCorrect, btn, 'showAnswerS1', 'm4_cp5_latihan_1', 'Jawaban Soal 1 tepat sekali. Jarak lurusnya adalah 50 meter.', 15);
}
...
function showAnswerS1() {
    ...
    if (typeof updateProgress === 'function') updateProgress('materi_4_penerapan_pythagoras', 'm4_cp5_latihan_1');
}
...
function cekSoal2() {
    ...
    if (!checkCompleteness(inputs)) return processValidation('s2', false);
    ...
    processValidation('s2', true, isAllCorrect, btn, 'showAnswerS2', 'm4_cp6_latihan_2', 'Tinggi menara berhasil dijawab dengan benar yaitu 7 meter.', 15);
}
...
function showAnswerS2() {
    ...
    if (typeof updateProgress === 'function') updateProgress('materi_4_penerapan_pythagoras', 'm4_cp6_latihan_2');
}
...
function cekSoal3() {
    ...
    if (!checkCompleteness(inputs)) return processValidation('s3', false);
    ...
    processValidation('s3', true, isAllCorrect, btn, 'showAnswerS3', 'm4_cp7_latihan_3', 'Analisis logika Soal 3 diselesaikan dengan sempurna!', 15);
}
...
function showAnswerS3() {
    ...
    if (typeof updateProgress === 'function') updateProgress('materi_4_penerapan_pythagoras', 'm4_cp7_latihan_3');
}

// ==========================================
// 6. VALIDASI REFLEKSI & SIMPAN PROGRESS
// ==========================================
function cekRefleksiPenerapan() {
    ...
    Swal.fire({
        ...
        icon: 'success',
        html: `Refleksi pemahamanmu berhasil disimpan. Selamat, kamu telah menuntaskan seluruh materi Bab Teorema Pythagoras!<br><br><span style="color: #198754; font-size: 1.3em; font-weight: bold;">🪙 +15 Poin!</span>`,
        confirmButtonColor: '#198754',
        confirmButtonText: '<i class="fas fa-flag-checkered me-2"></i> Selesai',
        allowOutsideClick: false
    }).then((result) => {
        if (result.isConfirmed) {
            // Update Checkpoint ke-8 (Refleksi)
            if (typeof updateProgress === 'function') {
                updateProgress('materi_4_penerapan_pythagoras', 'm4_cp8_refleksi', 15);
            }
...
document.addEventListener('DOMContentLoaded', function () {
    if (typeof window.setupReviewMode === 'function') {
        ...
        // 8. Refleksi
        window.setupReviewMode('m4_cp8_refleksi', 'button[onclick="cekRefleksiPenerapan()"]',
            function() {
